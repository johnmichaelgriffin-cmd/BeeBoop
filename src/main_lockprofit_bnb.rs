...
async fn main() -> Result<()> {
    ...
    info!("=== BEEBOOP LOCKPROFIT BNB 5M ===");
    ...
    tokio::spawn(feeds::binance::run_binance_feed_task_for(
        "bnbusdt", binance_tx.clone(), log_tx.clone(),
    ));
    ...
    // 4. RTDS / Chainlink — BNB
    tokio::spawn(feeds::rtds::run_rtds_task_for(
        "bnb/usd", chainlink_tx, chainlink_watch_tx, log_tx.clone(),
    ));
