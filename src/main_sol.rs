//! BeeBoSOL — Event-driven sniper bot for Polymarket SOL Up/Down.
//!
//! Same architecture as BooBeETH but for SOL markets.
...
async fn main() -> Result<()> {
    ...
    // SOL config overrides
    let mut config = Config::default();
    config.mode = mode;
    config.market = "sol".to_string();
    config.share_base = 10.0;           // 10sh per snipe
    config.max_pairs_per_window = 50;   // 50 snipes per window
    config.load_credentials();

    info!("=== BeeBoSOL v1.0.0 — SOL Sniper ===");
    ...
    // 1. Market discovery (SOL markets)
